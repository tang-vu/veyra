...
5. Any mutation, gap, reorder, or broken link in the journal is detected by verification.
...
| Approval replay or post-preview mutation | Single-use nonce, expiry, transaction binding, canonical effect digest repeated in request/grant/execution                                                                             | Compromise of the human principal or local daemon is out of scope                                                                                            |
...
| Duplicate execution after retry          | Unique durable idempotency reservation and serialized transaction operation; stored result returned once known                                                                         | Crash after external effect but before durable outcome is unknowable and enters manual recovery                                                              |
...
| Forged receipt                           | HMAC-SHA-256 over canonical receipt body with a local random key; constant-time verification                                                                                           | An attacker reading the key can forge receipts; this is not hardware attestation or non-repudiation                                                          |
| Journal tampering or missing links       | Canonical hash chain with sequence/previous-hash verification plus a transactionally updated local tail count/hash anchor                                                              | An attacker who can rewrite the whole DB and its local anchor can construct a new chain; no remote transparency log exists                                   |
...
| Secret leakage                           | Secret-reference wire type, late resolver at adapter boundary, redaction on journal writes, credential-safe display errors, provider error body suppression                            | A malicious adapter or compromised process memory can read resolved values; public inputs are intentionally logged                                           |
